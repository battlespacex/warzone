...
function bindScrollClassToggles() {
    ...
    window.visualViewport?.addEventListener("resize", refreshScroller, { passive: true });
}
...
function renderMap(events) {
    ...
    clusters.forEach((cluster, index) => {
        ...
        marker.addEventListener("click", () => {
            ...
            const feedCard = document.querySelector(`[data-event-id="${focusEvent.id}"]`);
            if (feedCard) {
...
function renderCounters(events) {
    ...
    const uniqueCountries = new Set(
        events.map((e) => String(e.location_label || "").split(",").pop()?.trim()).filter(Boolean)
    );

    if (totalEl) totalEl.textContent = String(events.length);
    if (mappedEl) {
        mappedEl.textContent = String(
            events.filter((e) => Number.isFinite(Number(e.lat)) && Number.isFinite(Number(e.lon))).length
        );
    }
    if (latestEl) latestEl.textContent = events[0]?.occurred_at ? formatTime(events[0].occurred_at) : "--";
