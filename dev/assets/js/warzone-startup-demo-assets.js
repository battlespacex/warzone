...
function isAssetVisible(asset) {
    return getAssetNumber(asset, "visible", 1) !== 0;
}
...
function createAllEntities() {
    ...
    // 2. Moving assets (staggered + delayed start)
    MOVING_ASSETS.forEach((asset, i) => {
        setTimeout(() => {
            ...
            // ⛔ delay movement start (prevents lag spike)
...
export function refreshWarzoneStartupDemoAssets() {
    ...
    STATIC_ASSETS.forEach(updateStaticEntity);
    MOVING_ASSETS.forEach((asset) => updateMovingEntity(asset, state.moving.get(asset.id), 0));
    state.viewer.scene?.requestRender?.();
}
