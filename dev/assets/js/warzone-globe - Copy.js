...
async function fetchSeedEvents() {
    const response = await fetch("/assets/others/events.json", { cache: "no-store" });
    if (!response.ok) {
...
function animateImpactPulse(viewer, missileId, rings) {
    ...
    const tick = () => {
        ...
        // smooth final fade of the whole effect near the end
        ...
        for (let i = 0; i < rings.length; i += 1) {
            ...
            // slower growth so rings do not catch each other too fast
            const grow = easeOutCubic(t);
            const scale = lerp(minScale, maxScale, grow);

            // smoother per-ring fade
            let alpha;
            if (t < 0.14) {
...
export async function initWarzoneGlobe() {
    ...
    const seedEvents = await fetchSeedEvents();
    seedEvents.forEach((event) => addEventEntity(viewer, event));
